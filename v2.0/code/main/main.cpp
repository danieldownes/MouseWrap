...
void __fastcall TfrmMain::timControlTimer(TObject *Sender)
{
	...
	//////////////////////////
        ...
        //  (or simular; longhorn in mind) edge ...
...
{
	ShellExecute(0, "open", "http://www.Ex-D.net/products/utils/mousewrap/", NULL, NULL, SW_SHOWMAXIMIZED);
}
//---------------------------------------------------------------------------
...
void ToggleStatus(void)
{
        if(frmMain->timControl->Enabled)
        {
                ...
                frmMain->TrayIcon->Hint = "OFF : Mouse Wrap 2 : Ex-D Software Development(TM)";
                frmMain->mnuTurn->Caption = "&Turn ON";
        }
        else
        {
                frmMain->TrayIcon->SetIconIndex(0);
                frmMain->TrayIcon->Hint = "ON : Mouse Wrap 2 : Ex-D Software Development(TM)";
